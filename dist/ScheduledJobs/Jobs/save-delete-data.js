...
const getSaveDataForSingleUser = async (city, fiqah) => {
    if (!city || !fiqah)
        throw new Error('City or Fiqah is Missing');
    ...
    const weeklyData = await axios.get(`${apiEndPoint}city=${city}&school=${school}`);
    if (weeklyData.length <= 0)
        return;
    const [errAddingData] = await to(addPrayerTime({
